...
public class Memory extends SettingsPreferenceFragment {
    ...
    private boolean isExtStorageMounted = false;
    ...
    public void onCreate(Bundle icicle) {
        ...
        for (StorageVolume volume : storageVolumes) {
            if (!volume.isEmulated()) {
                /*****dengtonglong (11.23): identify some model and external SD card , let it disappear*******/
            	if(Build.PWV_CUSTOM_CUSTOM.equals("HKWS")){
            		if(!volume.getPath().contains("/sdcard1")){
    ...
    private void doUnmount() {
        ...
        try {
            ...
            /*****dengtonglong (11.23): identify some model and unmount SD card*******/
            if(Build.PWV_CUSTOM_CUSTOM.equals("HKWS") && isExtStorageMounted) {
                mountService.unmountVolume(sClickedMountPoint.replace("/sdcard0", "/sdcard1"), true, false);
                Log.v(TAG,"=======SD card unmount OK!========");
            }
            /******end******/
    ...
    private void mount() {
        ...
        try {
            if (mountService != null) {
                ...
                /*****dengtonglong (11.23): identify some model and mount SD card*******/
                if(Build.PWV_CUSTOM_CUSTOM.equals("HKWS") && isExtStorageMounted) {
                    mountService.mountVolume(sClickedMountPoint.replace("/sdcard0", "/sdcard1"));
                    Log.v(TAG,"=======SD card mount OK!========");
                }
                /******end******/
            } else {
