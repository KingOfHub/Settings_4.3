...
public class StorageVolumePreferenceCategory extends PreferenceCategory {
    ...
    /****dengtonglong (11.14): calculate  external SD card storage****/
    private long getSDTotalSize() {
        File path = Environment.getExternalStorageDirectory(); 
        String pathString=path.getPath().replace("/sdcard0", "/sdcard1");
        StatFs stat = new StatFs(pathString);     
		long blockSize = stat.getBlockSizeLong();  
		long totalBlocks = stat.getBlockCountLong(); 
		long AllBlocks =  blockSize * totalBlocks;		
        return (AllBlocks);  
    }
    /******end******/
    
    /****dengtonglong (11.14): calculate  external SD card available storage****/
    private long getSDAvailableSize() {
		...
		long AllBlocks =  blockSize * availableBlocks; 		
        return (AllBlocks);  
    }
    /******end******/ 
    ...
    public void updateDetails(MeasurementDetails details) {
        ...
        /****dengtonglong (11.14): internal SD card add external SD card storage****/
       if (mVolume == null) return;
       if( Build.PWV_CUSTOM_CUSTOM.equals("HKWS") && mVolume.getPath().contains("/sdcard0")){
    	    mItemTotal.setSummary(formatSize(details.totalSize+getSDTotalSize()));
            mItemAvailable.setSummary(formatSize(details.availSize+getSDAvailableSize()));
    	}
       /*********end********/       
    }
