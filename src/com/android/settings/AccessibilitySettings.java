...
public class AccessibilitySettings extends SettingsPreferenceFragment implements DialogCreatable,
    ...
    // zhouxin
    private static final String KEY_AUTO_POP_INPUT = "auto_pop_input";
    ...
    private CheckBoxPreference mAutoInputPre;
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (mToggleLargeTextPreference == preference) {
            ...
            return true;
        } else if (preference == mAutoInputPre){
      	    Global.putInt(getContentResolver(), Global.AUTO_POP_SOFTINPUT, mAutoInputPre.isChecked() ? 1 : 0);
        	return true;
        }
    ...
    private void initializeAllPreferences() {
        ...
        mAutoInputPre = (CheckBoxPreference) findPreference(KEY_AUTO_POP_INPUT);
        // zhouxin
        if (Build.PROJECT.equals("SQ26")&& Build.PWV_CUSTOM_CUSTOM.equals("SF")) {			
			try {
				mAutoInputPre.setChecked(Settings.Global.getInt(
						getContentResolver(), Global.AUTO_POP_SOFTINPUT) == 1);
			} catch (SettingNotFoundException e1) {
				// TODO Auto-generated catch block
				Log.i(TAG, "Settings not found Global.AUTO_POP_SOFTINPUT");
			}
		} else {
			removePreference(KEY_AUTO_POP_INPUT);
		}
