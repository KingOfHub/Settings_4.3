...
public class DevelopmentSettings extends PreferenceFragment
    ...
    private static final String DEBUG_DRAWING_CATEGORY = "debug_drawing_category";
    private static final String DEBUG_MONITORING_CATEGORY = "debug_monitoring_category";
    ...
    private static final String DEBUG_HW_DRAWING_CATEGORY_KEY = "debug_hw_drawing_category";
    ...
    boolean signed = SystemProperties.getBoolean("pwv.custom.sign", false);
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (Build.PWV_HIDE_PART_DEVELOPMENT) {
            removePartDevelopmentSettings();
        }
    }

    private void removePartDevelopmentSettings() {
        // Remove the preference
        Preference mbugreportr = findPreference(BUGREPORT);
        if (mbugreportr != null) {
            getPreferenceScreen().removePreference(mbugreportr);
        }
        if (mPassword != null) {
        ...
        if (mEnforceReadExternal != null) {
            getPreferenceScreen().removePreference(mEnforceReadExternal);
        }
        PreferenceCategory mdebuggingGroup = (PreferenceCategory) getPreferenceScreen()
                .findPreference(DEBUG_DEBUGGING_CATEGORY_KEY);
        if (mDebugAppPref != null) {
            mdebuggingGroup.removePreference(mDebugAppPref);
        }
        if (mWaitForDebugger != null) {
            mdebuggingGroup.removePreference(mWaitForDebugger);
        }
        if (mVerifyAppsOverUsb != null) {
            mdebuggingGroup.removePreference(mVerifyAppsOverUsb);
        }
        if (mAllowMockLocation != null) {
            mdebuggingGroup.removePreference(mAllowMockLocation);
        }
        if (mBugreportInPower != null) {
            mdebuggingGroup.removePreference(mBugreportInPower);
        }
       Preference mdebughwdrawingcategory = findPreference(DEBUG_HW_DRAWING_CATEGORY_KEY);
        if (mdebughwdrawingcategory != null) {
            getPreferenceScreen().removePreference(mdebughwdrawingcategory);
        }
       Preference mdebugdrawingcategory = findPreference(DEBUG_DRAWING_CATEGORY);
        if (mdebugdrawingcategory != null) {
            getPreferenceScreen().removePreference(mdebugdrawingcategory);
        }
       Preference mdebugmonitoringcategory = findPreference(DEBUG_MONITORING_CATEGORY);
        if (mdebugmonitoringcategory != null) {
            getPreferenceScreen().removePreference(mdebugmonitoringcategory);
        }
       Preference mdebugapplicationscategory = findPreference(DEBUG_APPLICATIONS_CATEGORY_KEY);
        if (mdebugapplicationscategory != null) {
            getPreferenceScreen().removePreference(mdebugapplicationscategory);
        }
        if (mFastPowerOn != null) {
            getPreferenceScreen().removePreference(mFastPowerOn);
        }
    }
    ...
    public void onResume() {
        ...
        if((Build.PWV_CUSTOM_CUSTOM.equals("KPOS") || Build.PWV_CUSTOM_CUSTOM.equals("KQ")||Build.PWV_CUSTOM_CUSTOM.equals("SYB") || (Build.PWV_CUSTOM_CUSTOM.equals("ABC") && Build.TYPE.equals("user")))) {
            if(getActivity().getIntent().getAction().equals("android.settings.APPLICATION_DEVELOPMENT_INTERNAL_SETTINGS") || !signed) {
                mEnabledSwitch.setChecked(mLastEnabledState);
                mEnabledSwitch.setEnabled(!Utils.isMonkeyRunning());
            } else {
