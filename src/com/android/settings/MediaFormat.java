...
public class MediaFormat extends Activity {
    ...
    private Button.OnClickListener mFinalClickListener = new Button.OnClickListener() {
            public void onClick(View v) {
                ...
                startService(intent);
                /*****dengtonglong (11.23): identify some model and format both SD card *******/
                if(Build.PWV_CUSTOM_CUSTOM.equals("HKWS")){
                    final StorageVolume[] storageVolumes = StorageManager.from(getApplicationContext()).getVolumeList();
                    for (int i = 0; i < storageVolumes.length; i++) {
                        if (storageVolumes[i].getPath().contains("/sdcard1")) {
                            intent.putExtra(StorageVolume.EXTRA_STORAGE_VOLUME, storageVolumes[i]);
                            startService(intent);
                        }
                    }
                }
