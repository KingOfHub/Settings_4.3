 ...
 */
public final class BluetoothSettings extends DeviceListPreferenceFragment implements Preference.OnPreferenceClickListener {
    ...
    private static final int MENU_ID_SHOW_SIMULATE_BTPRINTER = Menu.FIRST + 4;
    ...
    Preference mSimulatePrinterPreference;
    ...
    private void updateContent(int bluetoothState, boolean scanState) {
        ...
        switch (bluetoothState) {
            case BluetoothAdapter.STATE_ON:
             ...
             // Simulate printer device
                boolean support = SystemProperties.get("pwv.printer.support", "false").equals("true");
                if(support && mbWorkLonely) {
                    boolean enable = SystemProperties.getBoolean("persist.sys.btprinter.enable", true);
                    if (mSimulatePrinterPreference == null) {
                        mSimulatePrinterPreference = new Preference(getActivity());
                    }
                    mSimulatePrinterPreference.setTitle(BluetoothDevice.BLUETOOTH_NAME);
                    mSimulatePrinterPreference.setIcon(R.drawable.ic_bt_imaging);
                    mSimulatePrinterPreference.setPersistent(false);
                    //mSimulatePrinterPreference.setEnabled(true);
                    mSimulatePrinterPreference.setSummary(enable ?  R.string.bt_simulate_printer_disable : R.string.bt_simulate_printer_enable);
                    mSimulatePrinterPreference.setOnPreferenceClickListener(this);
                    preferenceScreen.addPreference(mSimulatePrinterPreference);
                }
    ...
    public void onScanningStateChanged(boolean started) {
        ...
        getActivity().invalidateOptionsMenu();
        if(mSimulatePrinterPreference != null && mLocalAdapter != null) {
            boolean isDiscovering = mLocalAdapter.isDiscovering();
            mSimulatePrinterPreference.setEnabled(!isDiscovering);
        }
    }
    ...
    public boolean onPreferenceClick(Preference preference) {
        ...
        if(mSimulatePrinterPreference != null && preference == mSimulatePrinterPreference) {
            Log.d(TAG, "onPreferenceClick mSimulatePrinterPreference");
            boolean enable = SystemProperties.getBoolean("persist.sys.btprinter.enable", true);
            SystemProperties.set("persist.sys.btprinter.enable", enable ? "false" : "true");
            mSimulatePrinterPreference.setSummary( enable ?  R.string.bt_simulate_printer_enable : R.string.bt_simulate_printer_disable);
            /*Intent btintent = new Intent("ACTION_BTPRINTER_SERVICE");
            btintent.putExtra("EXTRA_STATE", enable ? false : true);
            getActivity().sendBroadcast(btintent);*/
            Intent btintent = new Intent("android.intent.action.SIMULATE_PRINTER");
            if(enable) {
                getActivity().stopService(btintent);
            } else {
                getActivity().startService(btintent);
            }
            //updateContent(mLocalAdapter.getBluetoothState(), false);
            if (!enable && mLocalAdapter.getBluetoothState() == BluetoothAdapter.STATE_ON) {
                startScanning();
            }
        }
        return false;
    }
    boolean mbWorkLonely = false;
    private void checkServerPkgExist() {
        String targetPkg = "com.android.simulate.printer.server";
        try {
            PackageManager pkm = getActivity().getPackageManager();
            pkm.getPackageInfo(targetPkg, PackageManager.GET_SERVICES);
            mbWorkLonely = true;
        } catch (NameNotFoundException e) {
            Log.e(TAG, "Unnable to find Simulate printer ServerApk, work lonely!!");
            mbWorkLonely = false;
        }
    }
}
